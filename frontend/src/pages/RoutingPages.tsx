                          ...
                          <dl className="mt-1.5 grid gap-0.5 text-xs font-700 leading-5 text-slate-500">
                            {product.sku ? (
                              <div className="flex flex-wrap gap-x-1">
                                ...
                                <dd className="break-all">{product.sku}</dd>
                              </div>
                            ) : null}
                            <div className="flex flex-wrap gap-x-1">
                              <dt className="font-800 text-slate-600">Precio</dt>
                              <dd>{formatMoney(product.price)} por {product.unit || 'unidad'}</dd>
                            </div>
                            {[product.brand, product.category, product.subcategory].filter(Boolean).length > 0 ? (
                              <div className="flex flex-wrap gap-x-1">
                                <dt className="font-800 text-slate-600">Datos</dt>
                                <dd className="break-words">{[product.brand, product.category, product.subcategory].filter(Boolean).join(' / ')}</dd>
                              </div>
                            ) : null}
                          </dl>
                        </div>
